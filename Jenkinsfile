pipeline {
  ...
  stages {
    ...
    stage('Run dev container') {
      ...
      steps {
        sh 'docker run -p 3300:3000 -d --name eirbmon-front-dev -v /var/lib/docker/volumes/jenkins-data/_data/workspace/Game_dev/BuildInfo:/Front/src/game/Unity/BuildInfo/ eirbmon/front-dev'
    ...
    stage('Run prod container') {
      ...
      steps {
        sh 'docker run -p 3000:3000 -d --name eirbmon-front -v /var/lib/docker/volumes/jenkins-data/_data/workspace/Game_master/BuildInfo/:/Front/src/game/Unity/BuildInfo/ eirbmon/front'
